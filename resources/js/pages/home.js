const content = `
<div class="relative">
    <div class="lg:w-1xl sm:text-center lg:text-left">
        <h1 class="text-4xl tracking-tight font-bold text-gray-900 sm:text-5xl md:text-6xl">
            <span class="block xl:inline">Realisez ses projets !</span>
            <span class="block text-indigo-600 xl:inline">Tontine en ligne</span>
        </h1>
        <p class="mt-3 text-base text-gray-500 sm:mt-5 sm:text-lg sm:max-w-xl sm:mx-auto md:mt-5 md:text-xl lg:mx-0">La technologie au coeur de nos modes de vies, découvrez la gestion de vos tontines directement en ligne</p>
        <div class="mt-5 sm:mt-8 sm:flex sm:justify-center lg:justify-start">
            <div class="rounded-md shadow">
            <a href="#" class="w-full flex items-center justify-center px-8 py-3 border border-transparent text-base font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 md:py-4 md:text-lg md:px-10"> Get started </a>
            </div>
            <div class="mt-3 sm:mt-0 sm:ml-3">
                <a href="#" class="w-full flex items-center justify-center px-8 py-3 border border-transparent text-base font-medium rounded-md text-indigo-700 bg-indigo-100 hover:bg-indigo-200 md:py-4 md:text-lg md:px-10"> Live demo </a>
            </div>
        </div>
    </div>
        <div class="lg:absolute lg:inset-y-0 lg:right-0">
            <img class="h-56 w-full object-cover sm:h-72 md:h-96 lg:w-full lg:h-full" src="storage/images/banking.jpg" alt="">
        </div>
</div>

`
const home = () => {
    const main = document.querySelector("#main");
    main.innerHTML = content;
}


export default home;
